...
var createArchive = function(input, currentPath, callback) {
  ...
  if (process.platform == 'darwin') {
    ...
    zipCommand = "pushd . & cd " + currentPath + "/.." + " & 7za a -tzip " + input.appname + ".zip " + input.appname + " & rm -rf " + currentPath + " & popd";
  }
...
exports.convertToApp = function(inputData, assetPath, callback) {
  var process = function(input, currentPath) {
    ...
    var _renameFiles = [
      ...
      { src: path.join(currentPath, 'AppName.xcodeproj'), dest: path.join(currentPath, input.appname + '.xcodeproj')}
    ];
  ...
  currentPath = path.join(currentPath, Date.now().toString(), input.appname);
