...
exports.convertToApp = function(inputData, callback) {
  
  // var renameFiles = ['AppName-Info.plist', 'AppName-Prefix.pch','AppName', 'AppName.xcodeproj', ]
  
  var process = function(input, assetPath, currentPath) {
  ...
  exec(extractCommand, function(error, stdout, stderr) {
    ...
    if (assetPath)
      fs.renameSync(assetPath, path.join(currentPath, 'AppName', 'Resources'));
