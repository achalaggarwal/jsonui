...
var objectDependencies = function(type) {
  if (_objectDependencies[type]) {
    return _objectDependencies[type];
  }
  else {
    return [];
  }
...
var processObjectColors = function(objects) {
  ...
  for (var i = objects.length - 1; i >= 0; i--) {
    ...
    if (_object.objects !== undefined) {
      processObjectColors(_object.objects);
    }
    ...
    for (var j = _keys.length - 1; j >= 0; j--) {
      ...
      _object["NSbackgroundcolor"]  = _object["NSbackgroundcolor"] || "";
      _object["NSfontcolor"]        = _object["NSfontcolor"] || "";
    }
  }
};
...
var getIntegratedClassDependencies = function(objects) {
  ...
  for (var i = 0; i < objects.length; i++) {
    _object = objects[i];
    if (_objectDependencies[_object.type] === null) {
       continue;
    }
...
var makeObjectRecords = function(objects) {
  ...
  for (var i = 0; i < objects.length; i++) {
    ...
    if (_objectDependencies[_object.type] === null || typeof(_object.object_id) === 'undefined' ) {
      continue;
    }
    ...
    if (_object.objects !== undefined) {
      for (var k = _object.objects.length - 1; k >= 0; k--) {
        if (typeof(_object.objects[k].object_reference) === 'undefined') { continue; }
...
var makeBaseObjectRecords = function(objects) {
  ...
  for (var i = 0; i < objects.length; i++) {
    _object = objects[i];
    if (typeof(_object.object_reference) === 'undefined') { continue; }
...
exports.render = function(input, currentPath, callback) {
  ...
  if (is_iPad) {
    ...
    NSFrame_Device = "{{0, " + top_height + "}, {768, " + (1024 - top_height) + "}}";
    if (is_landscape) {
      NSFrame_Device = "{{0, " + top_height + "}, {1024, " + (768 - top_height) + "}}";
      landscape = 3;
    }
  }
  else {
    targetRuntimeIdentifier = "IBCocoaTouchFramework";
    NSFrame_Device = "{{0, " + top_height + "}, {320, " + (480 - top_height) + "}}";
    if (is_landscape) {
      NSFrame_Device = "{{0, " + top_height + "}, {480, " + (300 - top_height) + "}}";
      landscape = 3;
    }
  }
  ...
  renderThis(path.join(currentPath, 'FirstViewController.xib'), {
      is_iPad                     : is_iPad,
      landscape                   : landscape,
      is_Navbar                   : is_Navbar,
